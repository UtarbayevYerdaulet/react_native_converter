...
import ConvertScreenHeader from "../components/ConvertScreenHeader";
...
export default function ConvertScreenMain() {
  ...
  async function getCash() {
    ...
    for (const elem in res.data) {
      let name = res.data[elem].code;
      let value = res.data[elem].value;
      let date = res.meta.last_updated_at.split("T")[0];
      let time = res.meta.last_updated_at.split("T")[1].replaceAll("Z", "");
      ...
      if($.converter.currencies.includes(res.data[elem].code)) {
        let id = frontDb.length + 1;
        frontDb.push({ id, name, value, date, time });
      } else if(res.data[elem].code !== $.converter.currency) {
  ...
  return (
    <View>
      <View style={styles.header}>
        ...
        >
          <View style={{ justifyContent: "center", alignItems: "center" }}>
            <Text style={{ color: "white", fontSize: 16 }}>{$.converter.currency}</Text>
          </View>
      ...
      <FlatList
        style={styles.main}
        ListHeaderComponent={<ConvertScreenHeader />}
        data={dataBase}
